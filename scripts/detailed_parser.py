...
class ISUCalcFSParser:
    ...
    def _parse_events(self, root):
        ...
        for event in root.findall('.//Event'):
            ...
            # Если дата начала не указана, но есть дата окончания, используем дату окончания
    ...
    def _parse_persons(self, root):
        ...
        # Парсим только основные Person_Couple_Team из Participants_List
        for person in root.findall('.//Participants_List//Person_Couple_Team'):
            ...
            if person_type == 'PER':  # Single skater
                ...
                # ВАЖНО: Для пар/танцев устанавливаем пол 'P' (Пара) независимо от значения в XML
    ...
    def _parse_clubs(self, root):
        ...
        seen_clubs = set()  # Для отслеживания уже обработанных клубов
        
        for club in root.findall('.//Club'):
            # Получаем ID клуба
            club_id = club.get('PCT_ID')
            
            # Пропускаем пустые теги Club (например, <Club/> в Team_Members)
            if not club_id:
                continue
            
            # Пропускаем уже обработанные клубы
            ...
            # Добавляем клуб в список обработанных
