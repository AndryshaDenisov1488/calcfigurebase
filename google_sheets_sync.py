...
def export_to_google_sheets(spreadsheet_id=None):
    ...
    try:
        ...
        if width_batch_requests3:
            safe_api_call(spreadsheet.batch_update, {'requests': width_batch_requests3})
            rate_limit_delay()
        ...
        # Добавляем пометку о том, что подсчеты без МС и КМС
        summary_data.append(['Примечание: Подсчеты без МС и КМС (с 1 сп до 3 юношеского)', '', '', '', '', ''])
        summary_data.append(['', '', '', '', '', ''])
