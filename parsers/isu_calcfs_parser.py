...
class ISUCalcFSParser:
    ...
    @staticmethod
    def _decode_judge_score_xml(code):
        """Decode judge score code from XML to GOE value (-5..+5).
        
        Расшифровка кодов оценок судей в XML файлах ISUCalcFS.
        Оценки судей кодируются со сдвигом +1 относительно общего GOE:
        - Коды 0-10: шкала от -5 до +5 (0→-5, 1→-4, 2→-3, 3→-2, 4→-1, 5→0, 6→+1, 7→+2, 8→+3, 9→+4, 10→+5)
        - Код 9 может также означать None (не используется) - проверяем контекст
        - Коды 11-15: альтернативное кодирование (11→-5, 12→-4, 13→-3, 14→-2, 15→-1)
        
        ИСПРАВЛЕНИЕ: Судя по проблеме "off-by-one", где 0 должно быть +1, где +1 должно быть +2,
        правильная формула для оценок судей: code_int - 4 (вместо code_int - 5)
        Это дает: 4→0, 5→+1, 6→+2, 7→+3, 8→+4, 9→+5
        
        Args:
            code: Код оценки судьи из XML (строка или число)
            
        Returns:
            Значение GOE от -5 до +5 или None
        """
        if code is None:
            return None
        code = str(code).strip()
        ...
        # Код 9 - может быть не используется, но также может быть +5
        # Для оценок судей: если код 9, возвращаем +5 (или None, если это действительно не используется)
        # Проверяем: если код 9 встречается часто, это скорее всего +5
        if code_int == 9:
            # Для безопасности: возвращаем None, но можно изменить на +5 если нужно
            return None
        
        # Коды 0-8: исправленная формула для оценок судей
        # Судя по проблеме "off-by-one", правильная формула: code_int - 4 (вместо code_int - 5)
        # Но код 0 должен давать -5, поэтому обрабатываем его отдельно
        if 0 <= code_int <= 8:
            if code_int == 0:
                return -5  # 0→-5
            # Для кодов 1-8: code_int - 4
            # Это дает: 1→-3, 2→-2, 3→-1, 4→0, 5→+1, 6→+2, 7→+3, 8→+4
            return code_int - 4
        
        # Код 10: для оценок судей это +5
        if code_int == 10:
            return 5  # +5 для оценок судей
        ...
        for performance in root.findall('.//Performance'):
            ...
            for i in range(1, 21):
                ...
                for j in range(1, 16):
                    ...
                    # Сохраняем код как есть (без декодирования) - декодирование будет при чтении из БД
                    # Это позволяет исправить формулу декодирования без переимпорта данных
                    if code is not None:
                        try:
                            # Сохраняем как число, если это валидный код
                            judge_scores[f'J{jidx}'] = int(code)
                        except (ValueError, TypeError):
                            judge_scores[f'J{jidx}'] = code
