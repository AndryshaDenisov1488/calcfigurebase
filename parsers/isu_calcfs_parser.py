...
class ISUCalcFSParser:
    ...
    def _decode_judge_score_xml(code):
        ...
        Правильная формула декодирования (проверено на реальных данных):
        - Код 0: -5
        - Коды 1-7: code - 4 (1→-3, 2→-2, 3→-1, 4→0, 5→+1, 6→+2, 7→+3)
        - Код 8: +4 (предположительно, 8 - 4 = 4)
        - Код 9: None (не используется)
        - Код 10: +5 (предположительно)
        - Коды 11-12: code - 16 (11→-5, 12→-4)
        - Коды 13-14: code - 9 (13→+4, 14→+5) - специальная формула для положительных значений
        - Код 15: code - 16 (15→-1)
        
        Примеры из реальных XML:
        - PRF_E01J01="14" → +5 (14 - 9 = 5)
        - PRF_E02J01="13" → +4 (13 - 9 = 4)
        - PRF_E03J01="7" → +3 (7 - 4 = 3)
        - PRF_E04J01="6" → +2 (6 - 4 = 2)
        - PRF_E05J01="5" → +1 (5 - 4 = 1)
        - PRF_E06J01="4" → 0 (4 - 4 = 0)
        - PRF_E07J01="3" → -1 (3 - 4 = -1)
        - PRF_E08J01="2" → -2 (2 - 4 = -2)
        - PRF_E09J01="1" → -3 (1 - 4 = -3)
        - PRF_E10J01="12" → -4 (12 - 16 = -4)
        - PRF_E01J01="11" → -5 (11 - 16 = -5)
    ...
    def _parse_performances(self, root):
        ...
        for performance in root.findall('.//Performance'):
            ...
            for i in range(1, 21):
                ...
                # Получаем информацию о половине программы (для бонуса 10%)
                half = performance.get(f'PRF_E{idx}HLF')  # 1 = первая половина, 2 = вторая половина
                wbp = performance.get(f'PRF_E{idx}WBP')  # 1 = бонус применен, 0 = нет бонуса
                if half:
                    judge_scores['half'] = int(half) if half.isdigit() else half
                if wbp:
                    judge_scores['wbp'] = int(wbp) if wbp.isdigit() else wbp
