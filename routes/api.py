    ...
    try:
        ...
        club_stats = db.session.query(
            ...
            db.func.count(db.distinct(case((
                ...
                Athlete.id
            ), else_=None))).label('athletes_with_free_participation'),
            ...
            db.func.count(case((
                ...
                1
            ), else_=None)).label('free_participations')
...
def api_athletes():
    ...
    if search and search.strip():
        ...
        if search_filter is None:
            logger.info(f"Поисковый фильтр не создан для запроса длиной {len(normalized)}")
